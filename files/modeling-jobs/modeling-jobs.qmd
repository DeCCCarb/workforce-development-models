...
```{r}
# Look at all options for well status
unique(wells$WellStatus)
```

```{r}
# Filter to idle wells
idle_wells <- wells |>
    filter(WellStatus == "Idle" &
            WellTypeLa == "Oil & Gas")

# Filter to idle wells in the Central Coast
...
Of the idle wells, 25,320 of them are of tyle Oil & Gas. In the central coast, its 3,149.

```{r}
# Calculate capping rate for a linear capping projection, starting with 0 wells capped at first year and increasing to all idle + active wells capped in final year (this will only work if the projections end at 2045 or later, because of the assumption that all currently active wells will become idle within the scope of the analysis. The average lifespan of a well is 15-20 years, and if we end projections earlier than 20 years, we will be counting active wells that may not be idle yet)

# General formula will be capping_rate = idle_plus_active_wells / timeline_to_cap (ex: 2025-2045 = 20)
...
```{r}
ggplot(oil_capping_jobs) +
    geom_point(aes(x = year, y = total_jobs_created),
               color = "blue")
```

This analysis pretty much shows that to cap all 1977 idle and active wells in Santa Barbara county by 2045, there will be 494 jobs created, or about 25 annual jobs created for 20 years. 
...
### Create Land-Based Wind Jobs DataFrame
