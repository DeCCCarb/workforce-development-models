...
sb_roof_pv_om <- calculate_pv_om_jobs(
    ...
    direct_jobs = ((0.28 * 0.4) + (0.18 * 0.6)), # 0.22
    indirect_jobs = ((0.04 * 0.4) + (0.02 * 0.6)), # 0.028
    induced_jobs = ((0.02 * 0.4) + (0.01 * 0.6)) # 0.014
)
...
sb_roof_pv_const <- calculate_pv_construction_jobs(
    ...
    initial_capacity = 242.0159119,
    final_capacity = 1293.941196,
    direct_jobs = ((8.04 * 0.4) + (4.12 * 0.6)), # 5.688
    indirect_jobs = ((5.18 * 0.4) + (3.26 * 0.6)), # 4.028
    induced_jobs = ((2.71 * 0.4) + (1.61 * 0.6)) # 2.05
)
...
sb_roof_pv_om_low <- calculate_pv_om_jobs(
    ...
    direct_jobs = ((0.28 * 0.4) + (0.18 * 0.6)), 
    indirect_jobs = ((0.04 * 0.4) + (0.02 * 0.6)), 
    induced_jobs = ((0.02 * 0.4) + (0.01 * 0.6))
)
...
sb_roof_pv_const_low <- calculate_pv_construction_jobs(
    ...
    initial_capacity = 242.0159119,
    final_capacity = 776.3647,
    direct_jobs = ((8.04 * 0.4) + (4.12 * 0.6)),
    indirect_jobs = ((5.18 * 0.4) + (3.26 * 0.6)),
    induced_jobs = ((2.71 * 0.4) + (1.61 * 0.6))
)
...
slo_roof_pv_const <- calculate_pv_construction_jobs(
    ...
    initial_capacity = 344.8405982,
    final_capacity = 1843.694708,
    direct_jobs = ((8.52 * 0.4) + (4.39 * 0.6)), # 6.042
    indirect_jobs = ((5.89 * 0.4) + (3.68 * 0.6)), # 4.564
    induced_jobs = ((2.54 * 0.4) + (1.49 * 0.6)) # 1.91
)
...
slo_roof_pv_const_low <- calculate_pv_construction_jobs(
    ...
    initial_capacity = 344.8405982,
    final_capacity = 1106.217,
    direct_jobs = ((8.52 * 0.4) + (4.39 * 0.6)),
    indirect_jobs = ((5.89 * 0.4) + (3.68 * 0.6)),
    induced_jobs = ((2.54 * 0.4) + (1.49 * 0.6))
)
...
ventura_roof_pv_const <- calculate_pv_construction_jobs(
    ...
    initial_capacity = 424.1984954,
    final_capacity = 3026.377541,
    direct_jobs = ((8.33 * 0.4) + (4.29 * 0.6)), # 5.906
    indirect_jobs = ((5.11 * 0.4) + (3.2 * 0.6)), # 3.964
    induced_jobs = ((2.68 * 0.4) + (1.59 * 0.6)) # 2.026
)
...
ventura_roof_pv_const_low <- calculate_pv_construction_jobs(
    ...
    initial_capacity = 424.1984954,
    final_capacity = 1815.827,
    direct_jobs = ((8.33 * 0.4) + (4.29 * 0.6)),
    indirect_jobs = ((5.11 * 0.4) + (3.2 * 0.6)),
    induced_jobs = ((2.68 * 0.4) + (1.59 * 0.6))
)
...
```{r}
job_projections <- read_csv(here('data','county_oil_employment_projections.csv')) %>% 
    filter(county %in% c('Santa Barbara','San Luis Obispo','Ventura'))
```

Now that we have all of the data for our three counties, let's filter to the scenario of a 2500 ft setback, reference case for oil_price_scenario, no excise tax, and no_prod_quota_scenario. This is the scenario that is closest to a business as normal approach 

```{r}
job_projections_current <- job_projections %>% 
    ...
    filter(year %in% c(2025:2045))
```

```{r}
job_projections_5280 <- job_projections %>% 
    filter(excise_tax_scenario == 'no tax') %>% 
...
We can now visualize our three counties' direct employment for the years 2019-2023. This is the time chunk that BLS has their data. So first we are going to look at those years. Once we confirm that will be okay, we will visualize the rest of the data, showing the projections up through 2045. Let's try using a stacked bar chart. 
```{r}
...
Okay, now that we've visualized the current fossil fuel employment, we can visualize the projections through 2045.

First we'll use the scenario that is most closely aligned to our current policy situation: a 2500 ft setback, reference case for oil_price_scenario, no excise tax, and no_prod_quota_scenario

```{r}
projections_plot1 <- job_projections_current %>% 
    group_by(county) %>% 
    ggplot(aes(x = year, y = total_emp, fill = county)) +
    geom_bar(stat = 'identity') +
    scale_fill_manual(values = c('Ventura' = '#4a4e69', 'Santa Barbara' = '#9a8c98', 'San Luis Obispo' = '#f0e68c')) +
    labs(title = 'Direct fossil fuel employment phaseout 2025-2045: 2500 ft setback policy',
         y = 'Total direct employment') +
    theme_minimal() +
    theme(axis.title.x = element_blank())
```

Next we'll see if there was a more ambitious scenario: 5280 ft setback, reference case for oil_price_scenario, no excise tax, and no_prod_quota_scenario

```{r}
projections_plot2 <- job_projections_5280 %>% 
    group_by(county) %>% 
    ggplot(aes(x = year, y = total_emp, fill = county)) +
    geom_bar(stat = 'identity') +
    scale_fill_manual(values = c('Ventura' = '#4a4e69', 'Santa Barbara' = '#9a8c98', 'San Luis Obispo' = '#f0e68c')) +
    labs(title = 'Direct fossil fuel employment phaseout 2025-2045: 5280 ft setback policy',
         y = 'Total direct employment') +
    theme_minimal() +
    theme(axis.title.x = element_blank())
```

Okay, now that we've tried it with a couple of scenarios, let's try to turn it into a function
```{r}
...
plot_employment_projection <- function(excise_tax = 'no tax', setback = 'setback_2500ft',
...
ggsave(here::here('plots', 'pv_base_plot.png'), pv_all_plot1, width = 8, height = 6)
...
```{r}
total_cap_line <- osw_all |>
    filter(year %in% c(2030:2045))
annual_cap_line <- osw_all |>
...
Specific number of job results - Utility Solar
```{r}
rooftop_jobs_sb <- pv_all %>% 
    filter(county == "SB" &
               technology == "Rooftop PV" &
               year == "2045" &
               ambition == "High" &
...
```{r}
# low ambition 2045 goal utility solar dodged by county
pv_2045_low <- pv_all %>% 
    filter(county %in% c("SLO", "SB", "V") &
               technology == "Utility PV" &
               ambition == "Low" &
               type == 'direct' &
               occupation %in% c('O&M', 'Construction') &
...
pv_2045_low_plot <- ggplot(pv_2045_low, aes(x = year, y = n_jobs, group = occupation)) +
    geom_col(position = position_dodge2(preserve = "single"), aes(group = interaction(county, occupation))) +
    facet_wrap(~county) +
    scale_fill_manual(labels = c("Construction Jobs", "Operations & Maintenance Jobs"),
                      values = c("#4a4e69", "#9a8c98")) +
    labs(title = "Projected direct jobs from Utility PV development",
     y = "FTE Jobs") +
    theme_minimal() +
    theme(
        axis.title.x = element_blank(),
...
## Plots for technical documentation
```{r}
utility_high <- pv_all %>% 
    filter(county %in% c("SLO", "SB", "V") &
               technology == "Utility PV" &
...
utility_high <- utility_high |>
    group_by(year, occupation, county) |>
    summarise(n_jobs = sum(n_jobs))

utility_high$county <- factor(utility_high$county, levels = c("SLO", "SB", "V"))



##### Utility Solar #####
# TOTAL TOTAL jobs (direct and indirect and induced)
total_util_jobs_facet <- ggplot(utility_high,
       aes(
           x = as.factor(year),
           y = round(n_jobs, 0),
           group = occupation
    ...
    scale_x_discrete(breaks = scales::breaks_pretty(n = 2)) +
    labs(
        title = "Total Projected Jobs From Utility Solar 2025 - 2045",
        y = "Full-time Equivalent Jobs"
    ) +
    ...
    theme(
        ...
        # Axes
        axis.title.x = element_blank(),
        axis.title.y = element_text(size = 16, margin = margin(t = 0, r = 10, b = 10, l = 0)),
        axis.text.x = element_text(size = 14, margin = margin(t = 10)),
        axis.text.y = element_text(face = "bold", size = 14, margin = margin(r = 5)),
...
ggsave(here::here('plots', 'technical-documentation', 'total_util_jobs_facet.png'), total_util_jobs_facet, width = 10, height = 6)

# Capacity
utility_high_cap <- utility_high |>
    group_by(county, year) |>
    summarise(cap = total_capacity_mw)

utility_high_cap$county <- factor(utility_high$county, levels = c("SLO", "SB", "V"))


# Plot
total_util_cap_facet <- ggplot(utility_high_cap,
       aes(x = as.factor(year), y = cap)) +
    geom_point(color = "#3A8398", size = 3) +
    scale_y_continuous(labels = scales::comma) +
    scale_x_discrete(breaks = scales::breaks_pretty(n = 2)) +
    labs(
        title = "Utility Solar Total Capacity Projections 2025 - 2045",
        y = "Capacity (MW)"
    ) +
...
ggsave(here::here('plots', 'technical-documentation', 'total_util_cap_facet.png'), total_util_cap_facet, width = 10, height = 6)
...
roof_high_jobs <-roof_high |>
    group_by(year, occupation, county) |>
    summarise(n_jobs = sum(n_jobs))

roof_high_jobs$county <- factor(roof_high_jobs$county, levels = c("V", "SLO", "SB"))

# TOTAL TOTAL jobs (direct and indirect and induced)
total_roof_jobs_facet <- ggplot(roof_high_jobs,
       aes(
           x = as.factor(year),
           y = round(n_jobs, 0),
           group = occupation
    ...
    scale_x_discrete(breaks = scales::breaks_pretty(n = 2)) +
    labs(
        title = "Total Projected Jobs From Rooftop Solar 2025 - 2045",
        y = "Full-time Equivalent Jobs"
    ) +
    theme_minimal(base_size = 14) +
    theme(
...
ggsave(here::here('plots', 'technical-documentation', 'total_roof_jobs_facet.png'), total_roof_jobs_facet, width = 10, height = 6)

# Capacity

# Summarize capacity and set facet order
roof_high_cap <- roof_high |>
    group_by(county, year) |>
    summarise(cap = total_capacity_mw, .groups = "drop")

roof_high_cap$county <- factor(roof_high_cap$county, levels = c("V", "SLO", "SB"))

# Plot
total_roof_cap_facet <- ggplot(roof_high_cap,
       aes(x = as.factor(year), y = cap)) +
    geom_point(color = "#3A8398", size = 3) +
    scale_y_continuous(labels = scales::comma) +
    scale_x_discrete(breaks = scales::breaks_pretty(n = 2)) +
    labs(
        title = "Rooftop Solar Total Capacity Projections 2025 - 2045",
        y = "Capacity (MW)"
    ) +
    facet_wrap(~county, scales = "free_y",
               labeller = labeller(county = c(
...
ggsave(here::here('plots', 'technical-documentation', 'total_roof_cap_facet.png'), total_roof_cap_facet, width = 10, height = 6)

```

```{r}
##### Floating Offshore Wind #####
osw_total_jobs <- osw_all |>
    filter(ambition == "High") |>
    group_by(year, occupation) |>
    summarise(n_jobs = sum(n_jobs))

total_osw_jobs <- ggplot(osw_total_jobs,
       aes(
           x = as.factor(year),
           y = round(n_jobs, 0),
           group = occupation
    ...
    scale_y_continuous(labels = scales::comma,
                       breaks = seq(from=2000, to=10000, by = 2000)) +
    scale_x_discrete(breaks = scales::breaks_pretty(n = 5)) +
    labs(title = "Total Projected Jobs From OSW 2025-2045",
        y = "Full-time Equivalent Jobs"
    ) +
     theme_minimal(base_size = 14) +
    theme(
...
ggsave(here::here('plots', 'technical-documentation', 'total_osw_jobs.png'), total_osw_jobs, width = 10, height = 6)

# Capacity 

osw_high_cap <- osw_all |>
    filter(ambition == "High") |>
    group_by(year) |>
    summarise(cap = total_capacity_gw, .groups = "drop")
...
total_osw_cap <- 
    ...
    scale_x_discrete(breaks = scales::breaks_pretty(n = 5)) +
    labs(
        title = "OSW Total Capacity Projections 2025-2045",
        y = "Capacity (GW)"
    ) +
...
ggsave(here::here('plots', 'technical-documentation', 'total_osw_cap.png'), total_osw_cap, width = 10, height = 6)

library(patchwork)

osw_td_plot <- total_osw_jobs + total_osw_cap

ggsave(here::here('plots', 'technical-documentation', 'osw_td_plot.png'), osw_td_plot, width = 11, height = 7)
```

```{r}
##### Oil Well Capping #####

# Join all oil capping counties
oil_capping <- rbind(oil_capping_jobs_sb,
...
sb_oil_cap_plot <- ggplot(oil_capping_jobs_sb) +
    geom_point(aes(x = as.factor(year), y = total_wells_capped), color = "#3A8398", size = 3) +
    geom_hline(yintercept = max_y, linetype = "dashed", color = "#3A8398") +
    annotate("text", x = 1, y = max_y, label = "Idle + active oil and gas wells", 
             hjust = 0, vjust = -0.5, size = 5, color = "#3A8398", fontface = "bold") +
    scale_x_discrete(breaks = scales::breaks_pretty(n = 5)) +
    scale_y_continuous(
        breaks = c(500,1000,1500,1977),
...
slo_oil_cap_plot <- ggplot(oil_capping_jobs_slo) +
    geom_point(aes(x = as.factor(year), y = total_wells_capped), color = "#3A8398", size = 3) +
    geom_hline(yintercept = max_y, linetype = "dashed", color = "#3A8398") +
    annotate("text", x = 1, y = max_y, label = "Idle + active oil and gas wells", 
             hjust = 0, vjust = -0.5, size = 5, color = "#3A8398", fontface = "bold") +
...
v_oil_cap_plot <- ggplot(oil_capping_jobs_v) +
    geom_point(aes(x = as.factor(year), y = total_wells_capped), color = "#3A8398", size = 3) +
    geom_hline(yintercept = max_y, linetype = "dashed", color = "#3A8398") +
    annotate("text", x = 1, y = max_y, label = "Idle + active oil and gas wells", 
             hjust = 0, vjust = -0.5, size = 5, color = "#3A8398", fontface = "bold") +
...
oil_capping_plot <- (v_oil_cap_plot + sb_oil_cap_plot + slo_oil_cap_plot) +
  plot_annotation(
    title = "Oil Well Capping Rate 2025–2045",
    theme = theme(
...
ggsave(here::here('plots', 'technical-documentation', 'oil_cap_rate_plot.png'), oil_capping_plot, width = 13, height = 7)

```

```{r}
# Jobs ----
oil_capping$county <- factor(oil_capping$county, levels = c("Ventura", "Santa Barbara", "San Luis Obispo"))

oil_cap_job_plot <- ggplot(oil_capping) +
    geom_point(aes(x = as.factor(year), y = total_jobs_created), color = "#3A8398", size = 3) +
    scale_x_discrete(breaks = scales::breaks_pretty(n = 5)) +
    scale_y_continuous(
    ...
    labs(
        title = "Total Jobs Created from Oil Well Capping",
        y = "Total Full-time Equivalent Jobs Created"
    ) +
    facet_wrap(
        ~county, scales = "free_y", 
       ) +
    theme_minimal(base_size = 14) +
    theme(
        # Title
...
ggsave(here::here('plots', 'technical-documentation', 'oil_cap_job_plot.png'), oil_cap_job_plot, width = 10, height = 7)
```
...
```{r}
# Combine and sum by year only
sum_and_label <- function(data, label) {
  data |>
    group_by(year) |>
    summarise(n_jobs = sum(n_jobs)) |>
    ungroup() |>
...
# Assign light grey to newly added tech, black for the rest
get_colors <- function(current_sources, new_source) {
  colors <- rep("#999999", length(current_sources))  # grey for current source
  names(colors) <- current_sources
  colors[new_source] <- "#000000"  # black for the newest source
  return(colors)
}

# Calculate total direct, indirect, and induced jobs for each tech
roof_total <- sum_and_label(roof_high_jobs, "Rooftop Solar")
utility_total <- sum_and_label(utility_high, "Utility Solar")
osw_total <- sum_and_label(osw_total_jobs, "Offshore Wind")

capping_total <- oil_capping |>
  group_by(year) |>
  summarise(n_jobs = sum(total_jobs_created)) |>
  ungroup() |>
  mutate(source = "Oil Well Capping")

# Sources per step
sources1 <- c("Rooftop Solar")
sources2 <- c("Rooftop Solar", "Utility Solar")
sources3 <- c("Rooftop Solar", "Utility Solar", "Offshore Wind")
sources4 <- c("Rooftop Solar", "Utility Solar", "Offshore Wind", "Oil Well Capping")

# Load League Spartan font to match presentation
font_add_google("League Spartan", "league")
showtext_auto()
showtext_opts(dpi = 300)

# Function to make plots
make_total_plot <- function(data, current_sources, new_source) {
  # Order so that new source is on the top of the stack, then reverse for ggplot stacking
  ordered_sources <- rev(c(setdiff(current_sources, new_source), new_source))
  data$source <- factor(data$source, levels = ordered_sources)
  
  ggplot(data, aes(x = as.factor(year), y = round(n_jobs), fill = source)) +
    geom_col() +
    scale_fill_manual(values = get_colors(current_sources, new_source)) +
    scale_y_continuous(limits = c(0,22500),
                       breaks = seq(0, 20000, by = 5000),
                       labels = scales::comma) +
    scale_x_discrete(breaks = seq(2025, 2045, by = 5)) +
    labs(x = NULL, y = "Full-Time Job Years") +
    theme_minimal(base_family = "league", base_size = 24) +
    theme(
      plot.title = element_blank(),
...
p1 <- make_total_plot(step1, sources1, "Rooftop Solar")        # Just rooftop
p2 <- make_total_plot(step2, sources2, "Utility Solar")        # Utility on top
p3 <- make_total_plot(step3, sources3, "Offshore Wind")        # Offshore Wind on top
p4 <- make_total_plot(step4, sources4, "Oil Well Capping")     # Capping on top


# Save plots
ggsave(here::here('plots', 'final-presentation', 'roof.png'), p1, width = 8, height = 8, dpi = 300)
ggsave(here::here('plots', 'final-presentation', 'roof_util.png'), p2, width = 8, height = 8, dpi = 300)
ggsave(here::here('plots', 'final-presentation', 'roof_util_osw.png'), p3, width = 8, height = 8, dpi = 300)
ggsave(here::here('plots', 'final-presentation', 'roof_util_osw_cap.png'), p4, width = 8, height = 8, dpi = 300)


# Blank plot
blank_data <- data.frame(year = factor(2025:2045), n_jobs = 0)

p0 <- ggplot(blank_data, aes(x = year, y = n_jobs)) +
  geom_blank() +
  scale_y_continuous(limits = c(0, 22500),
                     breaks = seq(0, 20000, by = 5000),
                     labels = scales::comma) +
...
ggsave(here::here('plots', 'final-presentation', 'blank.png'), p0, width = 8, height = 8, dpi = 300)
```


## Analysis for Lucas 4/21
...
write_csv(sb, here("data", "sb_direct_high.csv"))
```


```{r}
# Read in SB high ambition dataset
sb_direct_high <- read_csv(here("data", "sb_direct_high.csv"))

# Calculate total construction jobs for each tech ----

#osw
c_tot_osw <- sb_direct_high |>
    filter(technology == "Offshore Wind") |>
    filter(occupation == "Construction")

c_tot_osw = round(sum(c_tot_osw$n_jobs), 0)

# utility pv
c_tot_u_pv <- sb_direct_high |>
    filter(technology == "Utility PV") |>
    filter(occupation == "Construction")

c_tot_u_pv = round(sum(c_tot_u_pv$n_jobs), 0)

# rooftop pv
c_tot_r_pv <- sb_direct_high |>
    filter(technology == "Rooftop PV") |>
    filter(occupation == "Construction")

c_tot_r_pv = round(sum(c_tot_r_pv$n_jobs), 0)
```

## Analysis for td results report construction section (O&M filtering was done in console)

```{r}
# ran each variation for high and low ambition, and direct, indirect, and induced jobs
utility_constr_sum <- pv_all %>% 
    filter(county == 'SB' &
               technology == 'Utility PV' &
               ambition == 'Low' &
               type == 'direct' &
               occupation == 'Construction') %>% 
    mutate(total_jobs = sum(n_jobs))
```

```{r}
roof_constr_sum <- pv_all %>% 
    filter(county == 'SB' &
               technology == 'Rooftop PV' &
               ambition == 'Low' &
               type == 'direct' &
